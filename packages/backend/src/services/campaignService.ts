import { eq, and, inArray, gt, sql, isNull } from 'drizzle-orm'
    ...
    const page: Array<{ customerId: string; email: string | null; name: string | null; phone: string | null; pushSubscribed: boolean; customAttributes: unknown }>
      = hasRules(audienceFilter)
        ...
        ? await db
            ...
            .innerJoin(customers, eq(customers.id, customerSegments.customerId))
            .where(cursor
              ? and(eq(customerSegments.segmentId, campaign.segmentId), isNull(customers.mergedInto), gt(customers.id, cursor), excludeClause, ownerScope)
              : and(eq(customerSegments.segmentId, campaign.segmentId), isNull(customers.mergedInto), excludeClause, ownerScope))
            .orderBy(customers.id)
            .limit(pageLimit)
        : await db
            .select({
            ...
            .from(customers)
            .where(cursor
              ? and(eq(customers.projectId, campaign.projectId), isNull(customers.mergedInto), gt(customers.id, cursor), excludeClause, ownerScope)
              : and(eq(customers.projectId, campaign.projectId), isNull(customers.mergedInto), excludeClause, ownerScope))
            .orderBy(customers.id)
            .limit(pageLimit)
