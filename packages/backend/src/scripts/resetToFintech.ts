/**
 * Reset demo to fintech — wipes the old ecommerce project and seeds a fresh fintech one.
 *
 * Run: npx tsx src/scripts/resetToFintech.ts
 *
 * What it does:
 * 1. Deletes ALL existing projects for this demo (clean slate)
 * 2. Creates a fresh fintech project via the onboarding API
 * 3. Seeds 25 customers + 150+ fintech events
 * 4. Writes the new NEXT_PUBLIC_PROJECT_ID into packages/frontend/.env.local
 */
...
const ENV_LOCAL_PATH = resolve(__dirname, '../../../../packages/frontend/.env.local')
...
function updateEnvLocal(projectId: string): void {
  let content = ''
  try {
  ...
  if (content.includes('NEXT_PUBLIC_PROJECT_ID=')) {
    content = content.replace(/NEXT_PUBLIC_PROJECT_ID=.*/g, `NEXT_PUBLIC_PROJECT_ID=${projectId}`)
  } else {
...
async function reset() {
  console.log('🔄 Resetting demo to fintech...\n')
  console.log(`API Base: ${BASE_URL}\n`)

  // ─── Step 1: Delete existing demo projects ───
  console.log('Step 1: Clearing existing projects...')
  const existing = await apiGet('/api/onboarding/projects')
  if (existing.data?.length) {
    for (const p of existing.data) {
      await apiDelete(`/api/onboarding/projects/${p.id}`)
      console.log(`  ✓ Deleted project: ${p.name} (${p.id})`)
    }
  } else {
    console.log('  (no existing projects)')
  }

  // ─── Step 2: Create fresh fintech project ───
  console.log('\nStep 2: Creating fintech project...')
  ...
  for (const c of FINTECH_CUSTOMERS) {
    ...
    for (let i = 0; i < txnCount; i++) {
      ...
      const amount = type === 'debit' ? randomAmount(100, 50000) : randomAmount(5000, 200000)
      await apiPost('/api/v1/events', {
        event_name: 'transaction_completed',
        customer_id: c.id,
        timestamp: randomDate(30),
        idempotency_key: `txn_${c.id}_${i}_reset`,
        properties: {
          transaction_id: `TXN_${c.id}_${i}`,
          type, channel, amount, currency: 'INR',
          merchant: type === 'debit' ? randomChoice(['Swiggy', 'Amazon', 'Flipkart', 'BigBasket', 'Uber', 'PhonePe', 'GPay']) : 'Salary Credit',
          status: 'completed',
        },
        entities: [{ type: 'transaction', external_id: `TXN_${c.id}_${i}`, status: 'completed', attributes: { type, channel, amount, currency: 'INR' } }],
      }, authHeaders)
      eventCount++
    }
    ...
    for (let i = 0; i < loginCount; i++) {
      await apiPost('/api/v1/events', {
        ...
        properties: { platform: randomChoice(['android', 'ios', 'web']), session_duration_seconds: Math.floor(Math.random() * 600) + 30 },
      }, authHeaders)
      eventCount++
    }
    ...
    if (c.kyc === 'verified' && Math.random() > 0.4) {
      ...
      for (let i = 0; i < billCount; i++) {
        ...
        await apiPost('/api/v1/events', {
          ...
          customer_id: c.id,
          timestamp: randomDate(30),
          properties: { bill_type: billType, amount: randomAmount(200, 5000), biller_name: `${billType.replace('_', ' ')} provider`, payment_method: 'upi' },
        }, authHeaders)
        eventCount++
      }
    }
    ...
    if (c.kyc === 'verified') {
      ...
      await apiPost('/api/v1/events', {
        ...
        properties: { reason: 'periodic_review' },
      }, authHeaders)
      eventCount++
    }
    ...
    if (c.accountType === 'loan' || (c.kyc === 'verified' && Math.random() > 0.7)) {
      ...
      await apiPost('/api/v1/events', {
        ...
        entities: [{ type: 'loan', external_id: `LOAN_${c.id}`, status: 'active', attributes: { loan_type: loanType, principal: loanAmount } }],
      }, authHeaders)
      eventCount++
    ...
    if (c.accountType === 'demat' || c.balanceBracket === '25L+') {
      ...
      await apiPost('/api/v1/events', {
        ...
        properties: {
          sip_id: `SIP_${c.id}`,
          fund_name: randomChoice(['HDFC Top 100', 'SBI Bluechip', 'Axis Midcap', 'Mirae Asset Large Cap']),
          sip_amount: randomChoice([500000, 1000000, 2500000]),
          frequency: 'monthly',
          type: sipType,
        },
        entities: [{ type: 'sip', external_id: `SIP_${c.id}`, status: 'active', attributes: { type: sipType } }],
      }, authHeaders)
      eventCount++
    }
    ...
    if (c.accountType === 'credit_card') {
      await apiPost('/api/v1/events', {
        ...
        customer_id: c.id,
        timestamp: randomDate(60),
        properties: { card_type: 'credit', card_network: randomChoice(['visa', 'mastercard', 'rupay']), credit_limit: randomAmount(50000, 500000) },
      }, authHeaders)
      eventCount++
    }
  }
  console.log(`  ✓ ${eventCount} events sent`)

  // ─── Step 5: Update .env.local ───
  console.log('\nStep 5: Updating frontend .env.local...')
  updateEnvLocal(project.id)

  // ─── Summary ───
  console.log('\n' + '='.repeat(60))
  console.log('✅ Reset complete!\n')
  console.log(`Project ID:    ${project.id}`)
  console.log(`Domain:        fintech`)
  console.log(`Customers:     25`)
  console.log(`Events:        ${eventCount}`)
  console.log(`API Key:       ${api_keys.key_public}`)
  console.log('')
  console.log('Restart your Next.js dev server to pick up the new NEXT_PUBLIC_PROJECT_ID.')
}
