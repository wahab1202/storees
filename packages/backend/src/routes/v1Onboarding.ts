...
router.post('/projects/:id/archive', requireRole('admin'), requireProjectOwnership, async (req: Request, res: Response) => {
  try {
...
router.post('/projects/:id/unarchive', requireRole('admin'), requireProjectOwnership, async (req: Request, res: Response) => {
  try {
    const id = req.params.id as string
    const [p] = await db.select({ settings: projects.settings }).from(projects).where(eq(projects.id, id)).limit(1)
    if (!p) return res.status(404).json({ success: false, error: 'Project not found' })
