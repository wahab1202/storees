...
- **Connector-driven projects emit `order_placed`, not `order_completed`** — The `dataSyncService.buildOrderRow` synthesizes `order_placed` events from the connector. The ecommerce pack defaults prediction goals to `order_completed`, which results in `INSUFFICIENT_DATA: 0 positive labels` on every train. Either update goals (`UPDATE prediction_goals SET target_event='order_placed'`) or fix the pack to accept either name. Same logic applies to `checkout_completed` (not emitted) and `cart_abandoned` (no derivation source without `checkout_completed`).
- **PM2 caches env at first start — `pm2 restart` reuses cached env** — Plain `pm2 restart 1` keeps the env from when the process was first registered. After editing `.env`, use `pm2 restart 1 --update-env`. Better: configure `ecosystem.config.js` so PM2 reads `.env` on every restart.
- **`python-dotenv` loads from cwd, not project root** — When PM2 launches uvicorn from `packages/ml/`, a `packages/ml/.env` wins over a `<repo-root>/.env` of the same name. Having two `.env` files in different dirs is a footgun — if values diverge, the inner one silently wins and you'll waste hours debugging.
- **FastAPI's default 500 handler eats exception details** — Wrap each endpoint in a `try / except` that returns a structured response with `reason=f"{type(e).__name__}: {str(e)}"`. Generic 500s are debugging dead-ends; the actual exception type/message must reach the Node side or DB logs.
- **`insufficient_data` returns must populate `reason`** — Python `train()` initially returned `{status, n_positive, min_required}` without `reason`. The serve.py response model exposes `reason=result.get("reason")` which was always None. Operators couldn't tell whether they were short by 1 label or 1000. Always include a human-readable reason on failure paths.
- **Prediction scoring must use `ON CONFLICT UPDATE`, not plain INSERT** — Without dedup, every scheduled scoring run adds a new row per (project, goal, customer). After N days the score count exceeds the customer count. Migration 0058 + the unique index on `(project_id, goal_id, customer_id)` enforces this; the worker uses `onConflictDoUpdate` to keep one current score per customer.
- **Quality bucket labels must match what the AUC actually means** — Calling AUC 0.5–0.78 "Needs Data" makes working models look broken. 0.5–0.78 is honest predictive lift; only AUC < 0.5 is genuinely broken. Use "Fair" for the middle band and reserve "Needs Data" / red for sub-random AUCs.
- **Walk-forward validation is the right default for time-series propensity** — Most teams use random train/val split which leaks future info into the past. The Python `train_propensity.py` correctly uses walk-forward with non-overlapping prediction windows; falls back to random split only when the val window has too few positives.
- **Isotonic calibration needs a held-out slice with ≥10 positives** — `CalibratedClassifierCV(cv='prefit')` requires a separate cal set that isn't the training set. We use the temporally-most-recent 20% of train (min 50 rows, ≥10 positives) and skip calibration otherwise. Calibration must be wrapped in try/except since rare-class edge cases will trip it.
